...
class Canvas_interactif(tk.Canvas):
    ...
    def obtenir_zone_sous_clic(self, orig_x, orig_y):
        ...
        """
        zones_existantes = self.csv_helper.lire_zones(
            self.vehicule, self.motorisation, self.camera
        )
        ...
        for z in zones_existantes:
            ...
            if match_x is not None and match_y is not None:
                if (
                    match_x < orig_x < match_x + largeur
                    and match_y < orig_y < match_y + hauteur
                ):
                    return (
                        z_id,
                        match_x,
            ...
            if int(z["x0"]) < orig_x < int(z["x1"]) and int(z["y0"]) < orig_y < int(
                z["y1"]
            ):
                return (
                    z_id,
    ...
    def rafraichir_image(self):
        ...
        - None
        """
        if self.image_originale_cv is None:
            return
        ...
        img_resized = cv2.resize(
            self.image_originale_cv, (new_w, new_h), interpolation=cv2.INTER_AREA
        )
        ...
        self.config(
            scrollregion=(0, 0, max(self.canvas_w, new_w), max(self.canvas_h, new_h))
        )
    ...
    def dessiner_zones(self):
        ...
        zones_config = self.csv_helper.lire_zones(
            self.vehicule, self.motorisation, self.camera
        )
        scores = self.obtenir_scores_db()
                ...
                if z_id == "0":
                    ...
                    if score is not None:
                        ...
                        if match_x is not None and match_y is not None:
                            ...
                            self.create_text(
                                pos_x + largeur_zone / 2,
                                pos_y + hauteur_zone + 10,
                                text=texte,
                                fill=couleur,
                                tags=(
                                    "zone_text",
    ...
    def on_press(self, event):
        ...
        if self.app and not self.app.mode_admin:
            messagebox.showwarning(
                "Verrouillé", "Activez le mode modification pour interagir."
            )
            return
    ...
    def on_release(self, event):
        ...
        if hasattr(self, "zone_en_deplacement") and self.zone_en_deplacement:
            ...
            h_orig, w_orig = self.image_originale_cv.shape[:2]
            x0_reel, y0_reel = max(0, min(x0_reel, w_orig)), max(
                0, min(y0_reel, h_orig)
            )
            x1_reel, y1_reel = max(0, min(x1_reel, w_orig)), max(
                0, min(y1_reel, h_orig)
            )
            ...
            if (
                messagebox.askquestion(
                    "Ajouter référence",
                    f"Ajouter cette image comme référence pour la zone {z_id} ?",
                )
                == "yes"
            ):
                ...
                # Tracer dans la DB
                self.db.add_reference_to_db(
                    self.vis, self.vehicule, self.camera, z_id, x0_reel, y0_reel
                )
        ...
        if mouvement_x > 10 and mouvement_y > 10:
            if (
                messagebox.askquestion(
                    "Ajouter", "Ajouter une nouvelle zone irréversible ?"
                )
                == "yes"
            ):
        ...
        for z in zones_existantes:
            if z["type"] == self.variante_active:
                ...
                if chevauchement:
                    messagebox.showwarning(
                        "Collision",
                        f"La zone chevauche avec la zone {z['numero_zone']}",
                    )
                    self.delete(self.rect)
                    return

        new_id = self.obtenir_prochain_id_zone(
            self.vehicule, self.motorisation, self.variante_active
        )
    ...
    def action_ajouter_reference(self):
        ...
        if resultat_clic:
            ...
            if (
                messagebox.askquestion(
                    "Ajouter Référence",
                    f"Ajouter image de référence pour la zone {z_id} ?",
                )
                == "yes"
            ):
                cropped_img = self.image_originale_cv[y0_reel:y1_reel, x0_reel:x1_reel]
    ...
    def obtenir_prochain_id_zone(self, vehicule, motorisation, type_actuel):
        ...
        zones_cam = [
            z for z in zones if str(z.get("numero_camera")) == str(self.camera)
        ]
        ...
        ids_utilises_vehicule = set(
            int(z["numero_zone"])
            for z in zones
            if str(z.get("numero_zone", "")).isdigit()
        )
    ...
    def afficher_popup_nom(self):
        ...
        combo = ttk.Combobox(
            fenetre, textvariable=nom_var, values=liste_noms, state="readonly"
        )
        ...
        tk.Button(btn_frame, text="Valider", command=valider).pack(
            side=tk.LEFT, padx=10
        )
        tk.Button(btn_frame, text="Annuler", command=annuler).pack(
            side=tk.LEFT, padx=10
        )
    ...
    def on_right_click(self, event):
        ...
        if self.app and not self.app.mode_admin:
            messagebox.showwarning(
                "Verrouillé", "Activez le mode modification pour supprimer une zone."
            )
            return
        orig_x = int((self.canvasx(event.x) - self.offset_x) / self.ratio)
        orig_y = int((self.canvasy(event.y) - self.offset_y) / self.ratio)
        ...
        if resultat_clic:
            ...
            if messagebox.askyesno(
                "Supprimer la zone",
                f"Voulez-vous vraiment supprimer définitivement la Zone {z_id} ?\n\nCela effacera également toutes ses images de référence.",
            ):
                self.csv_helper.supprimer_zone(
                    self.vehicule, self.motorisation, self.camera, z_id
                )
                target_dir = self._get_target_directory(z_type)
                ...
                print(
                    f"[Canvas] Zone {z_id} supprimée avec succès (CSV et {len(fichiers_ref)} image(s) effacée(s))."
                )
                if self.app and self.app.infos_vehicule_actuel:
                    for info_cam in self.app.infos_vehicule_actuel:
                        if str(info_cam["camera_source"]) == str(self.camera):
